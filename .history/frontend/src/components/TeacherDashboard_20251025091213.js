...
const TeacherDashboard = () => {
  ...
  useEffect(() => {
    ...
    const initializeData = async () => {
      ...
      try {
        ...
        await Promise.all([fetchAllStudents(), fetchStudentStats()]); // Then students
      } catch (err) {
  ...
  const fetchRooms = async () => {
    ...
    try {
      ...
      const mappedRooms = response.data.map(room => ({
        id: room.id,
        title: room.title, // Use title directly
        subject: room.subject,
        description: room.description,
        room_code: room.room_code, // Use room_code directly
  ...
  const fetchStudentStats = async () => {
    ...
    try {
      const response = await axios.get(`${API_BASE_URL}/api/teacher/students/stats`, { // Fixed endpoint
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const createRoom = async (e) => {
    ...
    const roomData = {
      title: newRoom.title.trim(), // Changed from name to title
