...
const StudySession = () => {
  ...
  const [pdfRefreshKey, setPdfRefreshKey] = useState(0); // Force PDF viewer refresh
  ...
  const handlePdfUpload = async (event) => {
    ...
    try {
      ...
      // Upload to teacher's endpoint if user is teacher, otherwise we need a student upload endpoint
      // For now, we'll use the teacher endpoint structure
      await axios.post(
      ...
      setPdfRefreshKey(prev => prev + 1); // Trigger PDF viewer refresh
      addAlert('Study material uploaded successfully!', 'success');
      setTimeout(() => setPdfSuccess(''), 3000);
    } catch (error) {
      console.error('Upload PDF error:', error);
      setPdfError(error.response?.data?.detail || 'Failed to upload PDF');
      setTimeout(() => setPdfError(''), 3000);
    } finally {
  ...
  // Socket.IO and other logic remains the same...
  useEffect(() => {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* Metrics Panel - keeping original code */}
        {showMetricsPanel && (
          <div style={{
            background: 'rgba(255, 255, 255, 0.95)',
            borderRadius: '16px',
