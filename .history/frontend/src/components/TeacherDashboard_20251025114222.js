...
const TeacherDashboard = () => {
  ...
  useEffect(() => {
    console.log('TeacherDashboard useEffect triggered', { user, hasInitialized: hasInitialized.current, hasNavigated: hasNavigated.current });
    ...
    console.log('Token check:', { token: token ? 'Present' : 'Missing', userExists: !!user, userRole: user?.role });

    // Early return if token is missing or user is invalid
    if (!token || !user || typeof user !== 'object' || user.role !== 'teacher') {
      console.log('Invalid token or user, preparing to redirect');
      if (!hasNavigated.current) {
        hasNavigated.current = true;
        console.log('Setting error and navigating to /teacher/login');
        // Defer setError to avoid immediate re-render
        setTimeout(() => setError('Please log in as a teacher'), 0);
        localStorage.removeItem('teacher_token');
        localStorage.removeItem('student_token');
        navigate('/teacher/login', { replace: true });
      } else {
        console.log('Navigation skipped: already navigated');
      }
      return;
    }
    ...
    const initializeData = async () => {
      ...
      try {
        ...
        if (err.response?.status === 401 && !hasNavigated.current) {
          hasNavigated.current = true;
          console.log('401 error, navigating to /teacher/login');
          // Defer setError to avoid immediate re-render
          setTimeout(() => setError('Session expired. Please log in again.'), 0);
          localStorage.removeItem('teacher_token');
          localStorage.removeItem('student_token');
          navigate('/teacher/login', { replace: true });
        } else if (!error) {
