...
const GoalCard = React.memo(({ goal, onStart, onDelete, onViewReport }) => {
  ...
  // CORRECTION: Added separate state for delete button hover to avoid direct DOM manipulation.
    ...
    >
      ...
      <button
        ...
        style={{
          ...
          top: '16px',
          right: '16px',
          // CORRECTION: Background color is now controlled by React state.
          background: isDeleteHovered ? '#fecaca' : '#fee2e2',
          color: '#dc2626',
          border: 'none',
          width: '32px',
          height: '32px',
        ...
        // CORRECTION: Replaced direct style manipulation with state updates.
...
const NewGoalModal = ({ isOpen, onClose, onSubmit }) => {
  ...
  const handleSubmit = (e) => {
    ...
    const daysUntilTarget = calculateDaysLeft(formData.targetDate) + 1; // Include today
      ...
      <div style={{
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{
            ...
            <button
              // CORRECTION: Added type="button" to prevent form submission.
              type="button"
              onClick={addSubject}
              style={{
                width: '100%',
          ...
          <div style={{ display: 'flex', gap: '12px', marginTop: '32px' }}>
            ...
            <button
              // CORRECTION: Explicitly set type="submit" for clarity.
              type="submit"
              style={{
                flex: 1,
                padding: '14px',
...
export default function StudyGoalsDashboard() {
  ...
  // CORRECTION: Added state to handle button hover effects in a React-friendly way.
  const [isCreateButtonHovered, setCreateButtonHovered] = useState(false);
  const [isFirstGoalButtonHovered, setFirstGoalButtonHovered] = useState(false);
  ...
  const stats = useMemo(() => ({
    ...
    // NOTE: These are hardcoded and would typically be fetched from an API.
    streak: 5,
    weeklyTarget: 20,
    weeklyProgress: 12
  }), [goals]);
  ...
  // CORRECTION: Wrapped with useCallback to prevent re-renders of memoized GoalCard components.
  ...
  // CORRECTION: Wrapped with useCallback for performance.
  const handleStartSession = useCallback(async (goal) => {
    try {
      ...
      // NOTE: For a better SPA experience, consider using 'useNavigate' from 'react-router-dom'
      // instead of a full page reload with window.location.href.
  ...
  // CORRECTION: Wrapped with useCallback for performance.
  const handleViewReport = useCallback((goal) => {
  ...
  const filteredGoals = useMemo(() => {
    return goals.filter(goal => {
      ...
      // NOTE: The 'subjects' tab does not have filter logic yet.
      // It currently shows all goals.
      if (activeTab === 'subjects') return true; 
      return true;
    });
  }, [goals, activeTab]);
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
          ...
          <button
            ...
            style={{
              ...
              boxShadow: '0 4px 15px rgba(102, 126, 234, 0.4)',
              transition: 'all 0.3s',
              // CORRECTION: Hover effect is now controlled by React state.
              transform: isCreateButtonHovered ? 'translateY(-2px)' : 'translateY(0)'
            }}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '32px', flexWrap: 'wrap' }}>
          ...
          <div style={{ position: 'relative', width: 140, height: 140 }}>
            <svg width="140" height="140" style={{ transform: 'rotate(-90deg)' }}>
              ...
              <circle
                ...
                strokeDashoffset={`${2 * Math.PI * 60 * (1 - (stats.weeklyProgress / (stats.weeklyTarget || 1)))}`}
            ...
            <div style={{
              ...
              <div style={{ fontSize: '1.75rem', fontWeight: '700', color: '#667eea' }}>
                {Math.round((stats.weeklyProgress / (stats.weeklyTarget || 1)) * 100)}%
              </div>
          ...
          <div style={{ flex: 1, minWidth: '250px' }}>
            ...
            <div style={{
              ...
              <div style={{
                width: `${(stats.weeklyProgress / (stats.weeklyTarget || 1)) * 100}%`,
                height: '100%',
            ...
            <p style={{ marginTop: '12px', fontSize: '0.875rem', color: '#6b7280' }}>
              Keep going! You need <strong>{Math.max(0, stats.weeklyTarget - stats.weeklyProgress)} more hours</strong> to reach your weekly target 💪
            </p>
          </div>
        </div>
      </div>
          ...
          <div style={{ 
            ...
            <button
              ...
              style={{
                ...
                boxShadow: '0 4px 15px rgba(102, 126, 234, 0.4)',
                transition: 'all 0.3s',
                transform: isFirstGoalButtonHovered ? 'translateY(-2px)' : 'translateY(0)'
              }}
              onMouseEnter={() => setFirstGoalButtonHovered(true)}
              onMouseLeave={() => setFirstGoalButtonHovered(false)}
            >
