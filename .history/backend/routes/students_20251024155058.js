...
router.get('/', authenticateToken, requireRole('teacher'), async (req, res) => {
  try {
    ...
    console.log(`✓ Fetched ${Object.keys(studentStats).length} students for teacher ${req.user.id}`);
...
// Get specific student info (teacher only)
router.get('/:studentId', authenticateToken, requireRole('teacher'), async (req, res) => {
  try {
    ...
    console.log(`✓ Fetched student info for ${studentId}`);
    res.json({
      id: student.id,
      name: student.name,
      email: student.email,
...
router.get('/:studentId/sessions', authenticateToken, requireRole('teacher'), async (req, res) => {
  try {
    const { studentId } = req.params;
    ...
    const teacherRooms = await req.db.collection('rooms').find({
    ...
    const rooms = await req.db.collection('rooms').find({ id: { $in: roomIds } }).toArray();
    ...
    console.log(`✓ Fetched ${sessionsWithRoomData.length} sessions for student ${studentId}`);
...
// Get student progress (student or teacher)
router.get('/:studentId/progress', authenticateToken, async (req, res) => {
  try {
    const { studentId } = req.params;
    ...
    if (req.user.role === 'teacher') {
      ...
      roomIds = teacherRooms.map((room) => room.id);
    } else {
      const rooms = await req.db.collection('rooms').find({
        allowed_students: studentId,
      }).toArray();
    ...
    if (sessions.length === 0) {
      return res.json({
        ...
        achievements: [
          { title: 'Study Streak', description: '7 days in a row!', icon: '🔥', earned: false },
    ...
    console.log(`✓ Fetched progress for student ${studentId}`);
    res.json({
      ...
      achievements: [
        { title: 'Study Streak', description: '7 days in a row!', icon: '🔥', earned: totalSessions >= 7 },
        {
          title: 'Focus Master',
          description: 'Maintained 80%+ attention for 1 hour',
          icon: '🎯',
          earned: avgAttention >= 80 && totalStudyTime >= 60,
        },
        {
          title: 'Early Bird',
          description: 'Studied before 8 AM',
          icon: '🌅',
          earned: sessions.some((s) => new Date(s.start_time).getHours() < 8),
        },
        {
          title: 'Night Owl',
          description: 'Studied after 10 PM',
          icon: '🦉',
          earned: sessions.some((s) => new Date(s.start_time).getHours() >= 22),
        },
      ],
    });
  } catch (error) {
...
// Get student goals (student only)
router.get('/goals', authenticateToken, requireRole('student'), async (req, res) => {
  try {
    ...
    console.log(`✓ Fetched ${goalsWithProgress.length} goals for student ${req.user.id}`);
...
router.post('/goals', authenticateToken, requireRole('student'), async (req, res) => {
  try {
    ...
    if (!subject || !targetMinutes || !period || !startDate || !endDate) {
    ...
    console.log(`✓ Goal created for student ${req.user.id}: ${subject}`);
    res.json({
      ...goal,
      studiedMinutes: 0,
      progress: 0,
...
router.delete('/goals/:goalId', authenticateToken, requireRole('student'), async (req, res) => {
  try {
    ...
    console.log(`✓ Goal ${goalId} deleted for student ${req.user.id}`);
